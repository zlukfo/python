...
class Company:
    entity = {}
    inn: str = field(compare=True)                  # inn - уникальный идентификатор компании, по нему будем сравнивать экземпляры на одинаковость
    name: str = field(compare=False, default='')    # Названия компании у нас сожет и не быть - это необязательный параметр
    address: str = field(default='', compare=False)
    other: str = field(default='', compare=False)  # сюда будут складываться все нераспознанные данные
